...
function [fidexmult,Xout, N_sources, confidence_lvl] = Adaptive_sources_FAST_IF(Sig,N_S,win_length,num,delta,L,relative_energy_th,N_sources_ref)
...
for iii=1:num+1
    ...
    % Siga=
    [max_energy,t_start]=max(Siga(floor(win_length/2)+1:end-floor(win_length/2)));
    t_start=t_start(1)+floor(win_length/2);
%     t_start=floor(win_length/2)+1;
    v_m=0;
    v_m_mean = 0;
    for i=1:2*L+1
...
%             FF=FF+fft(Sig(jj,t_start-floor(win_length/2):t_start+floor(win_length/2)).*window_rot(i,:),length(Sig));
        end
%         FF = abs(FF);
        ...
        if v_m<v
            ...
            v_m=v;
            v_m_mean = v_m_mean + v_m;
        end
    end
    v_oldd=v_m;   
    v_m_mean = v_m_mean/(2*L+1);
    %% Ori extansion
    % mean energy:
    if iii == 1
        last_energy=v_m_mean;
        sources_energy(iii) = v_m_mean;
    else
        if last_energy*relative_energy_th > v_m_mean
%             if N_sources_ref > N_sources
%                last_energy*relative_energy_th
%                v_m_mean
%                dummy = 0;
%             else
%                last_energy*relative_energy_th
...
%                dummy = 0;               
%             end
            confidence_lvl = v_m_mean - last_energy*relative_energy_th;   
%             return;
        else
%             last_energy = v_m;
            sources_energy(iii) = v_m_mean;
